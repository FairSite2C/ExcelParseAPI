...
{
    // https://stackoverflow.com/questions/39156147/configuring-azure-blob-access-in-asp-net-5
    ...
    public class Blobber
    {
        ...
        public Blobber()
        {
            string name = AppSettings.Storage.AccountName;

            var key = AppSettings.Storage.AccountKey;

            try
            {
                credentials = new StorageCredentials(name, key);
        ...
        public  async Task<string> Upload(MemoryStream stream, string containerName)
        {
            ...
            await SetContainer(containerName);
            
            string name = UniqueBlobName();

            CloudBlockBlob blob = 
                container.GetBlockBlobReference(name);
        ...
        public string UniqueBlobName()
        {
            ...
            } while (BlobExists(key));

            return key;
            
        }

        public async Task SetContainer(string name)
        {
        ...
        {
            return client.GetContainerReference(container.Name)
                .GetBlockBlobReference(name).Exists();
        }
